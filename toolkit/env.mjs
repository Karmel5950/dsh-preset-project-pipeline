// 实例生命周期管理:拉起/停止一个 dsh web 实例。
//   node toolkit/env.mjs up       确保 home 就绪并启动(幂等)
//   node toolkit/env.mjs down     停止实例
//   node toolkit/env.mjs status   查看状态
// 单环境默认:管理单实例(默认端口 3080,dsh-home ~/.dsh)。
// 双环境显式开启(仓库根 .plugindev-env.json 或 DSH_ENV):管理 test 实例
// (plugindev/.dsh-home + 默认 3081,可用 DSH_TEST_PORT 覆盖)。首次 up 会把
// 生产 settings.yaml 复制过来(该文件不含密钥明文;凭据不复制 —— 零 token 的
...
async function up() {
  if (await reachable({ baseUrl: env.apiBase })) {
    console.log(`✓ ${env.name} 实例已在线 ${env.apiBase}(无需重复启动)`);
    return;
  }
  ...
  if (portInUse(env.port)) {
    console.error(`✗ 端口 ${env.port} 已被其他进程占用(单环境用 DSH_API 换基址;双环境用 DSH_TEST_PORT 换 test 端口)`);
    process.exit(1);
  }
  await bootstrapHome();
  ...
  while (Date.now() < deadline) {
    if (await reachable({ baseUrl: env.apiBase })) {
      console.log(`✓ ${env.name} 实例就绪: ${env.apiBase}(日志: ${LOG_FILE})`);
...
async function down() {
  const pid = await readPid();
  if (!pidAlive(pid)) {
    console.log(`- 没有在跑的 ${env.name} 实例`);
    await writeFile(PID_FILE, '', 'utf8');
    return;
  }
  execSync(`taskkill /PID ${pid} /T /F`, { stdio: 'pipe', shell: 'cmd.exe' });
  await writeFile(PID_FILE, '', 'utf8');
  console.log(`✓ 已停止 ${env.name} 实例(pid ${pid} 含子进程)`);
}

async function status() {
  const pid = await readPid();
  const online = await reachable({ baseUrl: env.apiBase });
  console.log(`${env.name} 实例: ${online ? '在线' : '离线'}  ${env.apiBase}  pid=${pidAlive(pid) ? pid : '-'}  home=${env.dshHome}`);
