...
> - **preset**:版本锚在 `presets/project-pipeline/package.json`(当前 **v0.22.0**),变更史 [CHANGELOG](presets/project-pipeline/CHANGELOG.md);
> - **project-hub**:版本锚在 `host-plugins/project-hub/ui/package.json`(当前 **v0.4.0**)对应 `host-plugins/project-hub/CHANGELOG.md`(hub 根无 package.json,以 `ui/package.json` 为权威)。
>
...
- **host 插件变更(若装了)**:`cd <仓库根>/host-plugins/project-hub && node deploy.mjs`(单环境默认;双环境显式开启下发布到 prod 用 `--env prod --confirm-prod`)。
...
- **单环境(你的真实实例,外部主路径)**:按你 dsh 实例的方式重启(本仓库工具链不管理单环境实例生命周期)。
- **双环境 test 隔离实例(维护者内件)**:`cd <仓库根> && node toolkit/env.mjs down && node toolkit/env.mjs up`(需 sibling `dsh-runtime` 就位)。
...
# 看板(装了 host 插件后):curl http://127.0.0.1:3080/plugins/project-hub/api
#   → { "ok": true, "projects": [ ... ] }
```
